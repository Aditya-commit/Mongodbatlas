        ...
        <div className={`w-1/2 transition-transform duration-300 ease-in-out ${start ? 'scale-100' : 'scale-0'} bg-white rounded-lg overflow-hidden`}>
            ...
            <div className='px-7 py-6'>
                ...
                <h1 className='flex items-center gap-x-2 px-3 border border-yellow-100 py-4 bg-yellow-50'>
                    <AlertCircle style='text-yellow-600 text-2xl' />
                    <span className={`text-xl ${nunito_sans.className} text-yellow-600 font-[600]`}>Confirming this will permanently delete the document</span>
                </h1>

                <div className='pt-18 flex flex-col gap-y-5'>
                    
                    <p className={`px-1 ${nunito_sans.className} text-[18px] text-gray-800 font-[500]`}>Enter <i>permanently delete</i> to confirm deletion of document with _id <b>{searchParams.get('id')}</b></p>

                    <input type='text' placeholder='permanently delete' className={`outline-none border-2 border-gray-200 rounded px-3 w-full py-2 transition-colors duration-300 ease-in-out ${nunito_sans.className} text-lg focus-visible:border-black font-[500]`} value={inputValue} onChange={handleChange} />

                    <button className={`${nunito_sans.className} font-[600] border-2 mt-10 w-full text-center ${(loading || !enable) ? 'bg-red-300 border-transparent' : 'bg-red-500 border-transparent focus-visible:shadow-[0_0_1px_3px_red] focus-visible:border-white hover:bg-red-600 transiton-all duration-300 ease-in-out'} text-white text-[19px] py-2.5 rounded`} onClick={deleteDocument}>
                        {loading
                        ?
                        <>Deleting...</>
                        :

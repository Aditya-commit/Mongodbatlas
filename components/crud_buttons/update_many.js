...
const Updateone = () => {
	...
	const [nmodified , setNmodified] = useState(0);
			...
			if(Options.database_name == undefined || Options.database_name == null){
				...
				if(Options.collection_name == undefined || Options.collection_name == null){
					...
					if(regex.test(query)){
						...
						axios.post('api/updates/updatemany' , {
		...
		<>
			...
			?
				<div className='filter bg-green-300 text-center'>
					...
					<p className='text-green-600 text-sm font-mono'><span className='text-lg font-bold'>{nmodified}</span> documents updated successfully</p>
				</div>
			:
			<>
				{iderror
